package dev.rmaiun.sommocker.helpers
...
object RabbitHelper {
  ...
  def initRabbitRoutes[F[_]](rc: RabbitClient[F])(using
  ...
  F[Unit] = {
    ...
    channel.use { ch =>
      for
        _
      <- rc.declareQueue(DeclarationQueueConfig(inputQ, Durable, NonExclusive, NonAutoDelete, Map()))(ch)
      _
      <-
      rc.declareExchange(
      ...
      _
      <- rc.bindQueue(inputQ, botExchange, inRK)(ch)
      yield ()
    }
  }
